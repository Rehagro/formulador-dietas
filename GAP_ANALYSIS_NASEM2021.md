...
| 2026-05-19 | **Importação de XML de laudo de análise** (commits `7b8d8db`, `47e84bf`, `6bf2ac6`) | Aluno arrasta XML do laboratório (schema Standard_XML_Data — 3R Lab / Dairy One / CVAS / Rock River, mesmo do AMTS), sistema parseia, sugere template do banco NASEM por Feed Type code, abre modal de edição com valores do laudo + frações proteicas do template. `parseLabXML.ts` cobre 33 campos quantitativos + metadata (laboratório, número da amostra, fazenda, data). DFND 48h calculada do `uNDF240 + NDFkd` quando disponível (modelo cinético de 1ª ordem van Soest), com fallback para `NDFDom_IV_30hr` ou template. `kd_amido` (que era null em todos os 159 alimentos NASEM) finalmente vem dos laudos. Persiste `origem_laudo` em `Alimento` (laboratório, número, data, fazenda, campos_calculados) para rastreabilidade. UX validada pelo Rafael sem ajustes. |
