...
export default function TabelaIngredientesRacao({ racao, resultado, onChangeIngredientes }: Props) {
  ...
  const onKgDChange = (idx: number, kg_d: number) => {
    const novos = racao.ingredientes.map((ing, i) => i === idx ? { ...ing, kg_d } : ing);
    onChangeIngredientes(novos);
  };

  const onRemove = (idx: number) => {
    onChangeIngredientes(racao.ingredientes.filter((_, i) => i !== idx));
  };
    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-xs tabular-nums">
          ...
          <tbody>
            {resultado.ingredientes.map((ic, idx) => (
              <tr key={ic.alimento.nome} className="border-t border-gray-100 hover:bg-amber-50/30">
                ...
                <td className="px-2 py-1.5 text-right">
                  <input
                    ...
                    step="0.1"
                    value={ic.kg_d}
                    onChange={e => onKgDChange(idx, Number(e.target.value) || 0)}
                ...
                <td className="px-2 py-1.5 text-right font-bold text-amber-900 bg-amber-50/50">{num(ic.kg_batida, 1)}</td>
